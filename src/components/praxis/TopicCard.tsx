...
export function TopicCard({ topic }: TopicCardProps) {
  ...
  const formattedDate = new Date(topic.lastActiveAt).toLocaleDateString(
    "en-US",
    {
      month: "short",
      day: "numeric",
  ...
  return (
    <Link href={`/prototypes/praxis/${topic.id}`} className="topic">
      <div className="corner">
        {t(`statuses.${topic.status}` as "statuses.active")}
      </div>
      <h3>{topic.title}</h3>
      <p className="line-clamp-2">
        {/* Add a brief description if available, otherwise blank */}
      </p>
