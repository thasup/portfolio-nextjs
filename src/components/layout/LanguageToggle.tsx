...
export function LanguageToggle() {
  ...
  return (
    ...
    >
      <span className={cn('transition-colors', locale === 'en' ? 'text-foreground font-semibold' : 'text-muted-foreground')}>
        EN
      </span>
      <span className="text-muted-foreground/30">|</span>
      <span className={cn('transition-colors', locale === 'th' ? 'text-foreground font-semibold' : 'text-muted-foreground')}>
        TH
      </span>
    </button>
  )
