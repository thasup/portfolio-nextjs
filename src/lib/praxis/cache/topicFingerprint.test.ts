...
import { fingerprint, normaliseTopic, sameTopic, titleFromRawInput } from '@/lib/praxis/cache/topicFingerprint';
...
describe('normaliseTopic', () => {
  ...
  it('strips leading intent phrases', () => {
    ...
    expect(normaliseTopic('Help me with public speaking')).toBe('public speaking');
  });
...
describe('titleFromRawInput', () => {
  ...
  it('keeps short connectors lowercase except first word', () => {
    expect(titleFromRawInput('the art of negotiation')).toBe('The Art of Negotiation');
  });
