 ...
 */
import { JSON_ONLY_DIRECTIVE, clamp, localeDirective, renderLearner, renderUnit } from '@/lib/praxis/prompts/_shared';
import {
  LearnerContext,
  PraxisLocale,
  PromptFixture,
...
export function build(input: TemplateGeneratorInput): string {
  const { locale, topic, unit, learner, kind, regenerateNote, desiredTitle } = input;
