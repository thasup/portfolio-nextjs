 ...
 */
function buildJudgePrompt(subject: string, generated: string, criteria: ReadonlyArray<RubricCriterion>): string {
  const criteriaLines = criteria.map((c, i) => `  ${i + 1}. "${c}" — ${CRITERION_DESCRIPTIONS[c]}`).join('\n');
  return [
...
export async function judge(input: JudgeInput): Promise<JudgeOutput> {
  const prompt = buildJudgePrompt(input.subject, input.generated, input.criteria);
  ...
  const parsed = extractJson<{ scores?: Array<{ criterion?: string; score?: number; note?: string }> }>(
    res.content,
  );
  ...
  const scores: RubricScore[] = input.criteria.map((c) => {
    ...
    const score = typeof match?.score === 'number' ? Math.max(0, Math.min(3, Math.round(match.score))) : 0;
    return {
      criterion: c,
      score,
  ...
  const mean = scores.length ? scores.reduce((sum, s) => sum + s.score, 0) / scores.length : 0;
