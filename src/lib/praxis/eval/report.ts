...
function buildMarkdown(run: EvalRun): string {
  ...
  const outlineOk =
    outline && (outline.meanScoreAvg ?? 0) >= EXIT_BAR.OUTLINE_MIN_MEAN;
  const unitOk = unit && (unit.meanScoreAvg ?? 0) >= EXIT_BAR.UNIT_MIN_MEAN;
  const scopeOk =
    scope &&
    scope.total > 0 &&
    scope.passed / scope.total >= EXIT_BAR.SCOPE_MIN_PASS_RATE;
  ...
  lines.push("");
  lines.push(
    `- Outline ≥ ${EXIT_BAR.OUTLINE_MIN_MEAN}: ${outlineOk ? "PASS" : "FAIL"} (mean=${(outline?.meanScoreAvg ?? 0).toFixed(2)})`,
  );
  lines.push(
    `- Unit ≥ ${EXIT_BAR.UNIT_MIN_MEAN}: ${unitOk ? "PASS" : "FAIL"} (mean=${(unit?.meanScoreAvg ?? 0).toFixed(2)})`,
  );
  lines.push(
    `- Scope pass rate ≥ ${EXIT_BAR.SCOPE_MIN_PASS_RATE}: ${scopeOk ? "PASS" : "FAIL"} (${scope?.passed ?? 0}/${scope?.total ?? 0})`,
  );
  lines.push("");
  ...
  for (const [name, s] of stats) {
    const mean =
      typeof s.meanScoreAvg === "number" ? s.meanScoreAvg.toFixed(2) : "—";
    lines.push(
      `| ${name} | ${s.total} | ${s.passed} | ${mean} | ${s.inputTokens} | ${s.outputTokens} |`,
    );
  }
  lines.push("");
  ...
  for (const r of run.results) {
    ...
    lines.push(
      `| ${r.module} | ${r.fixtureName} | ${r.passed ? "✅" : "❌"} | ${mean} | ${summary} |`,
    );
  }
  lines.push("");
    ...
    for (const f of failures) {
      ...
      if (f.rubric?.length) {
        ...
        for (const s of f.rubric) {
          lines.push(
            `  - ${s.criterion}: ${s.score}/3 — ${s.note || "(no note)"}`,
          );
        }
      }
