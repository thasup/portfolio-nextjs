import {
  DistortionConfig,
  DistortionIntensity,
} from "@/components/glass/glass-types";
...
export function getDistortionConfig(
  ...
  return {
    ...
    scale:
      customConfig.scale !== undefined
        ? clampValue(customConfig.scale, 50, 250)
        : preset.scale,
    seed:
      customConfig.seed !== undefined
        ? clampValue(customConfig.seed, 1, 20)
        : preset.seed,
    specularConstant:
      customConfig.specularConstant !== undefined
        ? clampValue(customConfig.specularConstant, 0.5, 2.0)
        : preset.specularConstant,
    surfaceScale:
      customConfig.surfaceScale !== undefined
        ? clampValue(customConfig.surfaceScale, 1, 10)
        : preset.surfaceScale,
  };
}
...
export function shouldEnableDistortion(
