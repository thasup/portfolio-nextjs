...
export default async function MyBidsPage() {
  ...
  if (!member)
    return <AppPage>{anonShell("Sign in to see your bids.")}</AppPage>;

  const [bids, missions] = await Promise.all([
    listMyBids(member.id),
    listMissions(org.id, { limit: 200 }),
  ]);
  const missionById = new Map<string, MissionDTO>(
  ...
  return (
    <AppPage>
      ...
      >
        {bids.length} bid{bids.length !== 1 ? "s" : ""} across active and
        shortlisted missions
      </p>
